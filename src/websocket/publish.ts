...
/** Called once during bootstrap, after the Bun server is constructed. */
 ...
 * (including the sender, if any) via Bun's native pub/sub. Use this for events
...
export function roomSubscriberCount(roomId: string): number {
  if (!appRef) return 0;
  return appRef.subscriberCount(roomTopic(roomId));
}
