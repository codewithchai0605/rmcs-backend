...
import { generatePlayerId, generateSessionToken, generateGuestName } from "../utils/ids";
import { sanitizeName, normalizeAvatarId } from "../utils/sanitize";
    ...
    if (existing) {
      ...
      if (existing.connected && existing.ws && existing.ws !== ws) {
        ...
        try {
          ...
          existing.ws.close(4000, "Replaced by a new connection");
        } catch {
