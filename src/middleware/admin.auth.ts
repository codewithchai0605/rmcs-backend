...
/**
 * Bun.serve() has no Express-style middleware chain, so - matching the
 * existing `requireLiveSession` / `requireAdmin` pattern already used
 * inline in http/routes.ts - this is a plain function each protected route
 * calls at the top of its handler, before doing any work or touching the DB.
 */
...
export function authenticateAdmin(req: Request): AdminAuthContext {
