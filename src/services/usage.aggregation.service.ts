...
import { isValidDateString, kolkataDayRangeUtc, yesterdayKolkata } from "../utils/date";
...
export async function aggregateDailyUsage(dateStr: string = yesterdayKolkata()): Promise<IDailyUsage> {
    ...
    try {
        ...
        await upsertDailyUsage(dateStr, start, end, {
            ...
            turnUsageEgressBytes: 0,
            turnUsageIngressBytes: 0,
            totalEgressBytes: 0,
            status: "error",
...
async function upsertDailyUsage(
    ...
    fields: Pick<
        IDailyUsage,
        "callsUsageEgressBytes" | "turnUsageEgressBytes" | "turnUsageIngressBytes" | "totalEgressBytes" | "status"
    > &
